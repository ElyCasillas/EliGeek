...
//backticks
const crearNuevaLinea = (imagen, nombre_prod, precio_prod, descripcion, buscar_prod) => {
...
listaServices
    .listaProductos()
    .then((data) => {
        data.forEach(({ imagen, nombre_prod, precio_prod, descripcion}) => { 
        ...
        if(nombre.includes(buscarp)){
            ...
            const nuevaLinea = crearNuevaLinea(imagen, nombre_prod, precio_prod, descripcion, buscar_prod);
            div.appendChild(nuevaLinea); 
            }
        
    })
})
.catch((error) => alert("Oops! Error. Comuniquese con Material"));
